...
- (void) dealloc {
    ...
    @try {
        ...
        NSLog(@"Error string ===== %@",[NSString stringWithFormat:@"%@",exception]);
    }
    [super dealloc];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    NSDictionary *DataDictionary = [[NSDictionary alloc] initWithObjects:[[NSArray alloc] initWithObjects:[NSString stringWithFormat:@"%d",_hitLayer.PieTag], nil] forKeys:[[NSArray alloc] initWithObjects:@"PiechartSelectedObject", nil]];
    ...
    [_delegate RetunWebserviceDataWithSuccess:self ObjectCarrier:DataDictionary];
}

#pragma No need for these methods for now
...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGPointDistanceBetweenTwoPoints(point, _touchBegan) < 5) {
        ...
        if (_hitLayer.opacity == 0.5) {
            [_hitLayer setOpacity:1.0];
        } else {
            [_hitLayer setOpacity:0.5];
        }
