...
- (void)setMode:(ANDProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == ANDProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateIndicators {
    ...
    else if (mode == ANDProgressHUDModeCustomView && self.customView != nil){
...
+(NSInteger) setGetShowIndicatorWithDisplayStatus:(DISPLAY_STATES)state
{
    static NSInteger displayCounter = 0;
    @synchronized(self) {
        
        if (state == DISPLAY_STATE_NONE) {
            return displayCounter;
        }
        
        displayCounter = (state == DISPLAY_STATE_SHOW) ? displayCounter+1 : displayCounter -1;
...
+ (ANDProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated
{
    ...
    int counter = [self setGetShowIndicatorWithDisplayStatus:DISPLAY_STATE_NONE];
    if (counter != 0) {
        [lock unlock];
        return (ANDProgressHUD *) [view viewWithTag:12345];
    }
    
    [self setGetShowIndicatorWithDisplayStatus:DISPLAY_STATE_SHOW];
    [lock unlock];
    
	ANDProgressHUD *hud = [[ANDProgressHUD alloc] initWithView:view];
    hud.tag = 12345;
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated
{
    NSLock *lock =  [[NSLock alloc] init];
    [lock lock];
    int counter = [self setGetShowIndicatorWithDisplayStatus:DISPLAY_STATE_HIDE];
    if (counter != 0) {
        [lock unlock];
        return NO;
    }
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[ANDProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		ANDProgressHUD *HUD = (ANDProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"WGProgressHUDViewIsNillException" 
					format:@"The view used in the WGProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = ANDProgressHUDAnimationFade;
        self.mode = ANDProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)done {
    ...
    if(delegate != nil && [delegate conformsToProtocol:@protocol(ANDProgressHUDDelegate)]) {
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0;
    if (animated && animationType == ANDProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0;
        if (animationType == ANDProgressHUDAnimationZoom) {
